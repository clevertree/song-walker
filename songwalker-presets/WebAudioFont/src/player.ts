...
export default class WebAudioFontPlayer {
    ...
    queueChord(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[], duration: number, volume?: number, slides?: WaveSlide[][]): WaveEnvelope[] {
    ...
    queueStrumUp(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[], duration: number, volume?: number, slides?: WaveSlide[][]): WaveEnvelope[] {
    ...
    queueStrumDown(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[], duration: number, volume?: number, slides?: WaveSlide[][]): WaveEnvelope[] {
        pitches.sort(function (a, b) {
            return a - b;
        });
        return this.queueStrum(audioContext, target, preset, when, pitches, duration, volume, slides);
    ...
    queueStrum(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[], duration: number, volume?: number, slides?: WaveSlide[][]): WaveEnvelope[] {
        volume = this.limitVolume(volume);
    ...
    queueSnap(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitches: number[], duration: number, volume?: number, slides?: WaveSlide[][]): WaveEnvelope[] {
        volume = this.limitVolume(volume);
    ...
    // resumeContext(audioContext: BaseAudioContext) {
    // 	try {
    ...
    // 	}
    // }
    queueWaveTable(audioContext: BaseAudioContext, target: AudioNode, preset: WavePreset, when: number, pitch: number, duration: number, volume?: number, slides?: WaveSlide[]): WaveEnvelope | null {
    ...
    findEnvelope(audioContext: BaseAudioContext, target: AudioNode): WaveEnvelope {
    ...
    findZone(audioContext: BaseAudioContext, preset: WavePreset, pitch: number): WaveZone | null {
    ...
    cancelQueue(audioContext: BaseAudioContext) {
